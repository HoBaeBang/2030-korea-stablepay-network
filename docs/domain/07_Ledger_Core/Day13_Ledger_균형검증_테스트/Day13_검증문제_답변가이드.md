...
8. `AccountID`처럼 `ID`를 대문자로 쓰는 이유는 무엇인가?
...
### 8. `AccountID`처럼 `ID`를 대문자로 쓰는 이유

Go에서는 `ID`, `URL`, `HTTP` 같은 약어를 대문자로 유지하는 관례가 많습니다.

그래서 `AccountId`보다 `AccountID`, `TransactionId`보다 `TransactionID`가 더 자연스럽습니다.

이 관례를 맞춰두면 코드 자동완성, 테스트 코드, 이후 문서가 서로 어긋날 가능성이 줄어듭니다.
...
Day12의 필드명과 Day13 테스트 코드가 서로 맞아야 한다는 점을 이해한다.
```
